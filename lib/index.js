  ...
  app.post('/invite', json(), (req, res, next) => {
    ...
    if (channels) {
      ...
      if (!channels.includes(channel)) {
        return res
        .status(400)
        .json({ msg: 'Canal no permitido' })
      }
      chanId = slack.getChannelId(channel)
      if (!chanId) {
        return res
        .status(400)
        .json({ msg: `Canal no disponible "${channel}"` })
      }
    }
    ...
    if (!email) {
      return res
      .status(400)
      .json({ msg: 'No has introducido un email' })
    }

    if (!remail().test(email)) {
      return res
      .status(400)
      .json({ msg: 'Email incorrecto' })
    }

    if (coc && '1' != req.body.coc) {
      return res
      .status(400)
      .json({ msg: 'Es obligatorio aceptar los ToS' })
    }

    invite({ token, org, email, channel: chanId }, err => {
      if (err) {
        if (err.message === `Redigiriendo a Slack...`) {
          return res
          .status(303)
